...
public class CellLayout extends ViewGroup implements BubbleTextShadowHandler {
    ...
    @Thunk
    Rect[] mDragOutlines = new Rect[4];
    @Thunk
    float[] mDragOutlineAlphas = new float[mDragOutlines.length];
    ...
    @Thunk
    HashMap<CellLayout.LayoutParams, Animator> mReorderAnimators = new HashMap<>();
    @Thunk
    HashMap<View, ReorderPreviewAnimation> mShakeAnimators = new HashMap<>();
...
//        return bestXY;
    ...
    private int[] findNearestArea(int pixelX, int pixelY, int minSpanX, int minSpanY, int spanX,
                                  int spanY, boolean ignoreOccupied, int[] result, int[] resultSpan, int itemType) {
        lazyInitTempRectStack();
...
//        }
    ...
    private ItemConfiguration findReorderSolution(int pixelX, int pixelY, int minSpanX, int minSpanY,
                                                  ...
                                                  ItemConfiguration solution, int itemType) {
        ...
        if (!success) {
            ...
            if (spanX > minSpanX && (minSpanY == spanY || decX)) {
                return findReorderSolution(pixelX, pixelY, minSpanX, minSpanY, spanX - 1, spanY,
                        direction, dragView, false, solution, itemType);
            } else if (spanY > minSpanY) {
                return findReorderSolution(pixelX, pixelY, minSpanX, minSpanY, spanX, spanY - 1,
                        direction, dragView, true, solution, itemType);
            }
    ...
    private ItemConfiguration findConfigurationNoShuffle(int pixelX, int pixelY, int minSpanX, int minSpanY,
                                                         int spanX, int spanY, View dragView, ItemConfiguration solution, int itemType) {
        int[] result = new int[2];
    ...
    */
    private void getDirectionVectorForDrop(int dragViewCenterX, int dragViewCenterY, int spanX,
                                           int spanY, View dragView, int[] resultDirection, int itemType) {
        int[] targetDestination = new int[2];

        findNearestArea(dragViewCenterX, dragViewCenterY, spanX, spanY, targetDestination, itemType);
    ...
    boolean isNearestDropLocationOccupied(int pixelX, int pixelY, int spanX, int spanY,
                                          View dragView, int[] result, int itemType) {
        result = findNearestArea(pixelX, pixelY, spanX, spanY, result, itemType);
    ...
    boolean createAreaForResize(int cellX, int cellY, int spanX, int spanY,
                                View dragView, int[] direction, boolean commit, int itemType) {
        int[] pixelXY = new int[2];
        regionToCenterPoint(cellX, cellY, spanX, spanY, pixelXY);

        // First we determine if things have moved enough to cause a different layout
        ItemConfiguration swapSolution = findReorderSolution(pixelXY[0], pixelXY[1], spanX, spanY,
                spanX, spanY, direction, dragView, true, new ItemConfiguration(), itemType);
    ...
    int[] performReorder(int pixelX, int pixelY, int minSpanX, int minSpanY, int spanX, int spanY,
        ...
        if ((mode == MODE_ON_DROP || mode == MODE_ON_DROP_EXTERNAL || mode == MODE_ACCEPT_DROP)
            ...
            getDirectionVectorForDrop(pixelX, pixelY, spanX, spanY, dragView, mDirectionVector, itemType);
        ...
        ItemConfiguration swapSolution = findReorderSolution(pixelX, pixelY, minSpanX, minSpanY,
                spanX, spanY, mDirectionVector, dragView, true, new ItemConfiguration(), itemType);

        // We attempt the approach which doesn't shuffle views at all
        ItemConfiguration noShuffleSolution = findConfigurationNoShuffle(pixelX, pixelY, minSpanX,
                minSpanY, spanX, spanY, dragView, new ItemConfiguration(), itemType);
     ...
     */
    public boolean findCellForSpan(int[] cellXY, int spanX, int spanY, int itemType) {
        if (cellXY == null) {
            cellXY = new int[2];
        }
        return mOccupied.findVacantCell(cellXY, spanX, spanY, itemType);
    }
        ...
        for (int cellX = 0; cellX < getCountX(); cellX++) {
            for (int cellY = 0; cellY < getCountY(); cellY++) {
                ...
                if (findReorderSolution(cellPoint[0], cellPoint[1], itemInfo.minSpanX,
                        ...
                        true, new ItemConfiguration(), itemInfo.itemType).isSolution) {
                    return true;
                }
            }
        }
    ...
    public boolean isRegionVacant(int x, int y, int spanX, int spanY, int itemType) {
        return mOccupied.isRegionVacant(x, y, spanX, spanY, itemType);
    }
}
